...
def createElectroweakCorrections():
	...
	electroweakCorrectionsFile.write("\tdouble precision alphaOrig, betaOrig\n")
	...
	electroweakCorrectionsFile.write("\t\t\t\tread(InputScaleReadIn, *) InputScale\n")
	...
	electroweakCorrectionsFile.write("\t\t\t! Copy the original values of alpha and beta\n")
	...
	electroweakCorrectionsFile.write('\t\t\t\twrite (*,*) "InputScale: ", InputScaleReadIn\n')
	...
	for x in range(0, len(processDescriptionList)):
		...
		for y in range(0, (32 - len(processDescriptionList[x][1]))):
			starString += "*"
		starString2 = ""
		for y in range(0, (35 - len(previousDecayparticle))):
			starString2 += "*"
		...
		electroweakCorrectionsFile.write("\t\t\t\t\t! Get the original values of alpha and beta and calculate the angle-dependent parameters\n")
		...
		for z in Config.renScaleDefinitions:
			electroweakCorrectionsFile.write('\t\t\t\t\t\tif ("' + z + '" == trim(InputScaleReadIn)) then\n')
		...
		if decayHeaderNeed:
			...
			electroweakCorrectionsFile.write('\t\t\t\t\t\toutputFileContent = trim(outputFileContent) // " (mixing angles) ' + starString2 + '\\n"\n\n')

			electroweakCorrectionsFile.write('\t\t\t\t\t\t! Write the values of alpha and beta in the chosen scheme(s) to the output file\n')
