...
#include "looptools.h"
 ...
 amplitudes(8) = (0.015625D0*CKM11*CKMC11*EL2*(-1.D0*MU2*A0(MU2) + MU2*A0(MW2) - 1.D0*MU2*MW2*B0(x, MU2, MW2) + MU2*x*B0(x, MU2, &
  &MW2) + B0(x, MU2, MW2)*DBLE(MU**INT(4.D0))))/(MW2*PI2*SW2*x)

 amplitudes(9) = (0.015625D0*CKM21*CKMC21*EL2*(-1.D0*MC2*A0(MC2) + MC2*A0(MW2) - 1.D0*MC2*MW2*B0(x, MC2, MW2) + MC2*x*B0(x, MC2, &
  &MW2) + B0(x, MC2, MW2)*DBLE(MC**INT(4.D0))))/(MW2*PI2*SW2*x)

 amplitudes(10) = (0.015625D0*CKM31*CKMC31*EL2*(-1.D0*MT2*A0(MT2) + MT2*A0(MW2) - 1.D0*MT2*MW2*B0(x, MT2, MW2) + MT2*x*B0(x, MT2,&
 ...
 amplitudes(12) = (0.001736111111111111D0*EL2*(-9.D0*x + 12.D0*SW2*x - 9.D0*A0(MD2) + 12.D0*SW2*A0(MD2) + 9.D0*A0(MZ2) - 12.D0*SW&
  &2*A0(MZ2) + 9.D0*MD2*B0(x, MD2, MZ2) - 9.D0*MZ2*B0(x, MD2, MZ2) - 12.D0*MD2*SW2*B0(x, MD2, MZ2) + 12.D0*MZ2*SW2*B0(x, MD2, MZ2&
  &) + 9.D0*x*B0(x, MD2, MZ2) - 12.D0*SW2*x*B0(x, MD2, MZ2) - 4.D0*x*DBLE(SW**INT(4.D0)) - 4.D0*A0(MD2)*DBLE(SW**INT(4.D0)) + 4.D&
  &0*A0(MZ2)*DBLE(SW**INT(4.D0)) + 4.D0*MD2*B0(x, MD2, MZ2)*DBLE(SW**INT(4.D0)) - 4.D0*MZ2*B0(x, MD2, MZ2)*DBLE(SW**INT(4.D0)) + &
  &4.D0*x*B0(x, MD2, MZ2)*DBLE(SW**INT(4.D0))))/ (CW2*PI2*SW2*x)

 amplitudes(13) = (0.03125D0*CKM11*CKMC11*EL2*(-1.D0*x - 1.D0*A0(MU2) + A0(MW2) + MU2*B0(x, MU2, MW2) - 1.D0*MW2*B0(x, MU2, MW2) &
  &+ x*B0(x, MU2, MW2)))/(PI2*SW2*x)

 amplitudes(14) = (0.03125D0*CKM21*CKMC21*EL2*(-1.D0*x - 1.D0*A0(MC2) + A0(MW2) + MC2*B0(x, MC2, MW2) - 1.D0*MW2*B0(x, MC2, MW2) &
  &+ x*B0(x, MC2, MW2)))/(PI2*SW2*x)

 amplitudes(15) = (0.03125D0*CKM31*CKMC31*EL2*(-1.D0*x - 1.D0*A0(MT2) + A0(MW2) + MT2*B0(x, MT2, MW2) - 1.D0*MW2*B0(x, MT2, MW2) &
