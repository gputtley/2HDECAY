double precision function HHtoTauTauBarCT(x)
 use constants
 use counterterms
 implicit none
#include "looptools.h"
 integer, intent(in) :: x
 double precision :: totalAmplitude

 select case (x)
	case (1)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk3*dBeta1KanUsual() + &
			& Yuk1/Yuk2*dAlphaKanUsual() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (2)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk3*dBeta2KanUsual() + &
			& Yuk1/Yuk2*dAlphaKanUsual() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (3)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta1KanAlter() + &
			& Yuk1/Yuk2*dAlphaKanAlter() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (4)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta2KanAlter() + &
			& Yuk1/Yuk2*dAlphaKanAlter() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (5)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta1PinchPStar() + &
			& Yuk1/Yuk2*dAlphaPinchPStar() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (6)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta2PinchPStar() + &
			& Yuk1/Yuk2*dAlphaPinchPStar() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (7)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta1PinchOS() + &
			& Yuk1/Yuk2*dAlphaPinchOS() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (8)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBeta2PinchOS() + &
			& Yuk1/Yuk2*dAlphaPinchOS() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (9)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk3*dBetaProcDep1Usual() + &
			& Yuk1/Yuk2*dAlphaProcDep1Usual() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (10)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBetaProcDep1Alter() + &
			& Yuk1/Yuk2*dAlphaProcDep1Alter() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (11)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk3*dBetaProcDep2Usual() + &
			& Yuk1/Yuk2*dAlphaProcDep2Usual() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (12)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBetaProcDep2Alter() + &
			& Yuk1/Yuk2*dAlphaProcDep2Alter() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (13)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSUsual()/ML - dMW2Usual()/(2D0*MW2) + Yuk3*dBetaProcDep3Usual() + &
			& Yuk1/Yuk2*dAlphaProcDep3Usual() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSUsual()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
	case (14)
		totalAmplitude = ( dgAtMZ()/(EL/SW) + dMLOSAlter()/ML - dMW2Alter()/(2D0*MW2) + Yuk3*dBetaProcDep3Alter() + &
			& Yuk1/Yuk2*dAlphaProcDep3Alter() + dZHHHHOS()/2D0 + Yuk1/Yuk2 * dZh0HHOSAlter()/2D0 + dZTauTauOSLeft()/2D0 + &
			& dZTauTauOSRight()/2D0 ) * (0.5D0*EL2*(MHH2 - 4.D0*ML2)*ML2*DBLE(Yuk2**INT(2.D0)))/(MW2*SW2)
 end select

 HHtoTauTauBarCT = totalAmplitude
end function HHtoTauTauBarCT
